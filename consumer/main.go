...
func main() {
	...
	// Start Server
	go func() {
		log.Println("Starting consumer", srv.Addr)
	...
	go func() {
		...
		if producerURL == "" {
			log.Println("producer url is empty")
			return
		}
		producerURL = fmt.Sprintf("http://%v", producerURL)
		for {
			res, err := http.Get(producerURL)
			if err != nil {
				log.Println(err)
				<-time.After(time.Second * 4)
				continue
			}
			log.Println("producer response")
